...
8. 传统学生场景（sourceType=study 或 goalType=exam_prep）要写真实的学生处境：考试节点与时间压力（月考/期中/期末/模考/升学倒计时）、学习内容是课纲既定的而非自选目标、作业与练习是老师布置的、有家长和老师的期望压力、有同学同伴的比较环境；不要把学生故事写成职场式的"碎片时间自学"，也不要写成一帆风顺的优等生模板——真实的卡点常是"明明很努力但方法不对""一到考试就发挥不出水平""假期计划坚持不下来"
...
12. 如果 candidateDomains 或 candidatePersonas 为空或未提供，意味着不限候选池：请从真实世界的领域/身份分布中自由采样（含非职场场景：备考、带娃、健康习惯、兴趣学习、家庭信息整理等），任意选择与最近样本不重复的组合，不要自我限定在常见"数据分析/Excel/运营/市场"清单里
13. 真人不会一口气说完整个故事，所以 story 只需要区分"首轮最可能怎么说"和"被追问后才会补的关键细节"，不要重复设计额外层级结构
14. personaSeed 不能只是一组人口统计学字段，还要包含稳定的人格、情感、行为模式和元认知特征
15. story 必须与 persona 保持一致：说话习惯、受挫方式、求助方式、对抗方式、遗忘修正方式必须与 personaSeed 里的对应字段对齐
16. 你生成的每个 trait 都必须是"可在对话中观察到的"，而不是抽象空话
17. story 不仅要给目标，还要给这个故事会优先触发哪种行为模式或情绪压力点
18. 如果提供 existingPersonaSeed，默认是在"同一个人"上补故事，不允许偷偷换人；只能换情境、事件和表层求助表达；不要重写此人的核心身份与长期行为底色，输出里的 personaSeed 只允许补空缺、做轻量对齐，不能把 occupation、corePersonality、helpSeekingPattern、adversarialPattern 等核心字段改成另一套人
