...
| 指标 | 修前 | 一轮修后 | 二轮修后 | 三轮修后 |
|---|---|---|---|---|
| 横向溢出页 | 0 | 0 | 0 | 0 |
| 被裁切文本 | 0 | 0 | 0 | 0 |
| **档位字号非单调（越大屏字越小）** | **34 处** | **0 处** ✅ | 0 处 | 0 处 |
| 键盘不可达（cursor:pointer 无 role/tabindex） | 0 | 0 | 0 | 0 |
| 可点元素 <24px | 38 个页面×档位组合 | 19 个组合，剩余 136 条样本全部是 `<input type=checkbox>`（见 §1.3） | 19 个组合，剩余 **133 条样本经 `type` 字段复核全部是 checkbox**（见 §1.3） | 同（登记例外） |
| **表格行高 <40px** | skills-recon 4 行（37px） | 同（见 §2.2） | **0 处** ✅（37→43px，见 §2.2） | 0 处 |
| **字号档 >3 的页面×档位组合** | 78 | 81（最多 16 档） | 78–81（最多 16 档） | **20（最多 8 档）**，且**文本档外字号 0** ✅（见 §八） |
| **控制台错误 / 失败请求** | 60 / 60（全部 `/api/users/me` 401） | 60 / 60（见 §4.1） | **0 / 0** ✅（见 §4.1） | 0 / 0 |

> 「三轮修后」= 2026-09-24 按 §2.1 做的**文本档收敛**（三角色 token），详见 §八。
> 字号档计数随 dev 库数据浮动（78↔81，同一份代码两次复跑即可差 1）；三轮之后剩下的
> >3 都是**展示型字号**（KPI 数字/实体名/大图标）——它们按设计不占文本档位。
...
**仍未做**（见 §3.2 / §3.3 / §3.5 后半）：i18n、skill-workbench 侧栏入口、
PromptEval 悬空引用的 DB 清理策略。§2.1 与 §2.3（字号档 / 档位蔓延）已在**第三轮**做掉，见 §八。
...
## 八、第三轮：文本档收敛为三角色（2026-09-24 续，§2.1 + §2.3）

### 8.1 根因（本轮查清）

§2.1 此前被当成"页面乱写半像素值"，实测**根因在共享层**：`mk-primitives.css` 的档位块
自己就声明了 6~10 个字号（1440 档 6 个：12.5/13/13.5/14/14.5/15，2800 档 10 个），
而且**档位只覆盖一部分选择器**——没被覆盖的（`.mk-section__head h4` / `.mk-card__note` /
`.mk-field__opt` / `.mk-status__meta-link` …）落回基线值，在 1440 上就是第 4、第 5 档。
页面层再各写各的（588 条档位内字号声明 + 上千处 `var(--mk-fs-N)` **固定值**引用，
后者不随档位缩放，在 3840 上直接变成 12/12.5/14px 的档外字号）。

### 8.2 改法：三级角色 token（SPEC §1 重写）

| 角色 token | 承担的元素 | 基线 | 1440 | 1920/2000 | 2800/3600 |
|---|---|---|---|---|---|
...
1. **共享层**：基线声明改引用角色 token；**补齐档位覆盖**（此前漏掉的 13 个文本选择器）；
   2000 档的文本字号全删（1920 已全覆盖）；3600 档只留覆盖层专属字号（模态/抽屉标题）。
   档位块从「30 个选择器各写一个 px」变成「3 行 token 覆写」。
2. **页面层**：71 个文件 1973 处字号声明机械改写为角色 token（micro 1401 / body 437 /
   emphasis 135），保留展示型 38 处。规则：按声明所在上下文的三角色尺度，落在文本带内的
   归角色，`> emphasis × 1.15` 的（KPI 数字/实体名/大标题）原样保留。
3. **图标字符与 `code`**：⋯ / × / ↑ 等字符改用角色 token（随档位缩放）；`code` 加
   `body.admin-route code { font-size: inherit }` 压掉 `design-system.css` 给它的用户侧
   `--text-sm`（14px）——实测 skill-workbench 有 15 处档外字号来自它。

### 8.3 结果

| 指标 | 收敛前 | 收敛后 |
|---|---|---|
| 字号档 >3 的页面×档位组合 | 78–81（最多 16 档） | **20（最多 8 档）** |
| **文本档外字号**（新指标，见 8.4） | 未度量（旧指标把 KPI 数字也算进来） | **0** |
| 档位块里的 font-size 字面量 | 1440 档 6 种 / 2800 档 10 种 | **0**（只剩 3 行 token 覆写） |
| 页面层档位内字号字面量 | 588 条 | **0 条**（守卫规则 13 拦回潮） |
| 横向溢出 / 裁切 / 键盘不可达 / 档位非单调 / 表格行 <40px / 控制台错误 | 0 | 0（未回归） |

实测 85 次渲染里 **62 次恰好只有 3 个文本档**（其余为 4 档 14 次、5 档 1 次、7 档 4 次、8 档 1 次）；其余多出来的都是展示型（KPI 34px、实体名
28.5px 等）。真机截图复核（1440 / 2560 的 总览·用户与学习者·执行日志·对账·模型与接入·
虚拟学习者）：层级仍读得出来——卡片标题靠 700 字重而非字号与正文区分，KPI 数字仍显著。

### 8.4 新增的机器判据

- **巡检**：运行时读 `--mk-fs-micro/body/emphasis`（角色 token 在当前档位就是该档位的值），
  落在 `(0, emphasis]` 内又不是三个角色值的字号 = **档外文本字号**；`> emphasis` 的是展示型，
  单独计数。摘要行直接给「文本档合规：N 个组合 / 档外最多 M 种」。
- **守卫**：规则 11 扩展为同时比三个角色 token 的跨档单调性（有效字号 = 值 × zoom）；
  规则 13（新增，硬失败）——档位块内不得出现文本带内的字号字面量，字面量只允许展示型。
  两条都用注入探针验证过会触发。


## 九、复跑记录（2026-09-25，桌面端二端巡检轮）

全量复跑 `node scripts/admin-audit.mjs`（85 次渲染），结论与 §八 三轮收敛后基线一致：

| 指标 | 本轮 |
|---|---|
| 横向溢出 / 裁切文本 / 键盘不可达 / 档位非单调 / 表格行 <40px | 0（未回归） |
| 文本档外字号 | 0 |
| 字号档 >3 的页面×档位组合 | 18（上轮 20；均为展示型字号计入的口径差） |
| <24px 可点 | 全部落已知豁免：16px 原生复选框（§四 登记）、`<details>`「诊断信息」折叠头（19px，R5 技术细节折叠区）、瞬态 toast 关闭键 20px |
| 控制台错误 / 失败请求 | 60 条——**全部为管理端 API 的 HTTP 500**（goal-conversations 统计 / agents/logs / overview/stats / activity / teaching-sessions 等 9 个端点），复验时 9/9 全部 200。定性：巡检窗口与并行会话改 backend 的 ts-node-dev 重启窗口重叠的瞬时现象，非产品回归 |

同轮完成用户侧桌面端巡检（1440 全路由 + 1920 抽查 + 暗色目检，超出本文档范围，记于巡检会话）：桌面字号刻度未被 ≤1100 移动密度改造侵蚀（h1 28 / 行动卡标题 26 / KPI 30 / 统计数字 22-28），0 横向溢出、0 console 错误、暗色适配完好。

### 9.1 逐页目检补充（2026-09-25，用户要求「逐页逐个细节看」）

对 1440 截图做逐页人工目检（24 集真实渲染 12 页 + 23 集同页补看 3 页，覆盖 T1 列表 ×7、
T2 详情 ×1、T3 驾驶舱 ×1、T4 图形/表单 ×4、Overview 特例、错误态 ×4）：

- **结构统一（R1-R6）全部成立**：状态条三要素、pills 带计数、分段筛选 + 搜索 + 仅真实/含模拟、
  表格徽章语义色（R2）、失败行红左边、卡内吸底分页、`<details>` 技术细节折叠（R5）、
  虚拟学习者页头 ghost 次按钮（代码核实仅 1 个 `--primary`）。
- **观察项（不构成违规）**：①页面级失败（shell 错误边界整页接管、无侧栏）与页面内失败
  （卡内错误态 + 重试）两种错误形态并存——失败源层级不同，属 R3 允许的设计；
  ②audit-logs 动作列存在裸 API 路径（mono + POST 徽章，R5 允许的技术降级形态，
  可读性可再收敛，登记不修）；③detail-user 子卡空态为一句灰字而非 MkEmptyState
  （非页面级空态，登记不修）。
- **未覆盖**：24 集有 7 页因 500 窗口/冷启动只拍到骨架或错误卡（skills-recon、prompt-eval、
  execution-logs/cost、ops-center-export、ops-hub-announce、skill-design），其中 5 页已用
  23 集（REAL）补看结构；skill-design 两轮均未采到（入口非 `<a>`，见 §六）。
