...
在本定位下：**原始 12 问的"机制与理解"已基本做完**；剩下的是**少量可选深化 + 两处此前少做的
说明/分区**（Q6/Q7 专门说明文档、Q2 虚拟学习者"拆分"）。方法论层（双传感器、ZPD 支架、静默降级治理、
...
| 2 | 虚拟学习者**拆分** + 可视化看板 | 看板 + 保留率曲线已做；**"拆分"（独立分区）未做** | 拆分（可选） |
...
| 6 | **学习者中心**：通用 vs 特异 | 仅有分散说明（`LEARNER_MODEL_ARCHITECTURE` / 理论地图） | **专门说明文档（此前少做）** |
| 7 | 聚合/拆分/融合/评估"**怎么做到的**" | 架构说明 + 轻量真值发现 | **专门说明文档（此前少做）** |
...
**剩余可选（按北星排序）**：① Q6/Q7 专门说明文档；② Q2 虚拟学习者"拆分"；③ Q8 延迟锚题；
④ Q1 复习容量；⑤ Q11b 接线；⑥ Q10 视场景；⑦ 契约漂移清理；⑧ Q20 补权威单价（外部输入）。
...
### ① 现在就能做（北星内、无商业依赖）
- **Q6/Q7 专门说明文档**（此前少做）：一份"学习者中心（通用 vs 特异）+ 状态聚合/拆分/融合/评估怎么做到的"说明，与 Q5 的 `AGENT_SKILL_MANUAL.md` 同规格。
- **Q2 虚拟学习者"拆分"**：把虚拟学习者从内嵌 tab 独立成区（看板与数据已就绪）。
...
- **Q20 token 面板**：补权威单价即可出金额（外部财务输入）。
...
3. **Q8 测量深化**：延迟锚题复用。
4. **Q1 复习容量（替代排序）**。
5. **Q11b 接线**；**Q10 `accumulate`** 视场景。
6. **契约漂移清理**。
7. **Q20 token 面板**：补权威单价即可出金额。
