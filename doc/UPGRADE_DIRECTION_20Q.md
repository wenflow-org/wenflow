...
在本定位下：**原始 12 问的"机制与理解"大多已交代**（含 Q6/Q7 说明文档 `LEARNER_CENTER_AND_STATE_FUSION.md`）；
剩下的是**少量可选深化 + 半做项**（Q2「拆分」未做、Q7 融合未接线、Q10 运行时未做、Q11 严格路径未点亮）。方法论层（双传感器、ZPD 支架、静默降级治理、
...
| 2 | 虚拟学习者**拆分** + 可视化看板 | 看板/保留率曲线已做（`e5e9e6e5`，`MemoryReview` 场景）；**"拆分"（独立分区）未做**——`VirtualLearners.vue` 仍 2192 行、内部 tab 内嵌；原计划原文即"独立分区暂缓" | 拆分（可选） |
...
**剩余可选（按北星排序）**：① Q2 虚拟学习者"拆分"（独立分区）；② Q1 复习容量（等数据）；
③ Q7 真值发现接线（可选）；④ Q10 运行时自定义（视场景）；⑤ Q11 点亮严格路径（可选）；
...
- **处置**：看板 + 保留率曲线已完成 `e5e9e6e5`（落 `MemoryReview` 场景）；**"拆分"（独立分区）未做**——原计划即"独立分区暂缓"，`VirtualLearners.vue` 仍 2192 行 tab 内嵌。
...
### ① 现在就能做（北星内、无商业依赖）
- **Q2 虚拟学习者"拆分"（独立分区）**：把 `VirtualLearners.vue`（2192 行、内部 tab）拆出独立分区；看板/数据已就绪。
...
**剩余可选（北星内，非阻塞）**：
1. **Q2 虚拟学习者"拆分"**（独立分区；看板已就绪）。
2. **Q1 复习容量（替代排序）**。
3. **Q7 真值发现接线**（能力已实现，无生产消费者）。
4. **Q10 `accumulate` 运行时**（视场景）；**Q11 点亮严格路径**（core yaml 声明结构化字段，可选）。
