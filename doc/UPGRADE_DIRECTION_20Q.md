...
| 2 | 虚拟学习者**拆分** + 可视化看板 | ✅ 看板（`VirtualProfile` 记忆池 + 保留率曲线，`e5e9e6e5`）；✅ 代码拆分（`VirtualLearners.vue` 2192→775，拆 7 组件，`a9a7ac7e`）；✅ 独立成「虚拟实验」组（`fb11aa33`） | — |
...
- **处置**：✅ 完成：看板（`VirtualProfile` 记忆池 + 保留率曲线 `e5e9e6e5`）；代码拆分 `VirtualLearners.vue` 2192→775（`a9a7ac7e`）；独立成「虚拟实验」组（`fb11aa33`，含「批量实验」提升为独立页）。
