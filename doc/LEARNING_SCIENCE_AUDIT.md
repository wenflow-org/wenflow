...
**遗留（下一轮）**：① 保持曲线的"间隔"目前只有少数样本，仍不够拟合；② 账本与"日窗口"在跨 UTC 日界附近出现过一次"脚本预演 1 条 / 开课 0 条"的漂移（该轮验证因此空跑），已在 §3.14 定案；③ A（收窄入库）尚未做。
...
**诚实边界（现在还不能声称什么）**：
- 带只在有 **≥6 条代码裁决**样本时才会动作；当前生产/验证场次里检查点样本还很少 ⇒ 实际多为 `hold`
  （行为与改造前一致）——**"85% 目标闭环"是接线完成，不是已达成**；
- 简答通道的要点判定对措辞敏感（会低估成功率），故不计入带；
- 真正的有效性仍只能靠延迟/迁移后测（§6 / §8 E1–E3）判定。
