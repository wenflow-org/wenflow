...
> 方法：**代码即事实**（逐条读实现）＋**真实数据**（`backend/prisma/dev.db`，18 个虚拟学习者、182 节课）＋**文献对照**
> 证据分级：**【A】**= 本次调查亲自在代码/数据库复核；**【B】**= 子系统逐文件审计给出 `file:line`，未逐条复核
...
| 判断 | 结论 |
|---|---|
| 教学**回合级**适应 | **是真闭环，且实测在跑**（analysis → 下一步教学 → 再分析） |
| **记忆/间隔重复**级闭环 | **代码齐全，但零流量**：182 节课 **0 次课内温故**、**0 条复习结果**、**0 次 FSRS 落库**；根因是 3 处断点（计划被初始化覆盖 / 计划从未进入任何一次 LLM 调用 / 结果摘取被饿死） |
| **结果测量**层 | **缺失**：无延迟后测、无迁移测，"掌握"是 LLM 自述 → **系统当前不可证伪**（回答不了"有没有学会"） |
...
2. **真实数据**：对 `prisma/dev.db` 做了封闭查询（证据类型分布、会话 blob、memory_traces、learning_metrics），并对 18 个虚拟学习者逐人复算「到期点 / 温故计划 / 近期课是否带计划与结果」。
...
| 环 | 内容 | 延迟 | 触发方式 | 实测状态 |
|---|---|---|---|---|
| **R1** | 回合内教学适应（analysis → 下一步/追问/支架） | 同回合~下一回合 | 自动 | ✅ **在跑**（每回合都有） |
| **R2** | **课内温故**（计划 → 检索 → 结果 → FSRS/预算） | 同课 | 自动 | ❌ **零流量**（详见 §3.2） |
...
| 功能提交（`360c861`，2026-09-15T20:14+08）之后开的课 | **5 节，全部 0 计划**（含 2026-09-16 一节） |

### 3.3 断点二：到期积压与"每课 1 条"【A】

对 18 个虚拟学习者复算 `buildReviewPlan`：
...
**46–104 条到期，每节课只能温故 1 条**（单条估算负担 1.3–1.95 已吃满 2.0 预算；`MAX_WARMUP_ITEMS=3` 是上限而非实际值）。
按每天 1 节课算，积压以**每天几十条**的速度增长——这正是代码注释声称要避免的 "Anki backlog death spiral"，但当前参数组合下它在结构上必然发生。
...
⇒ 效果度量（`relieved` / `still_triggered` / A-B 对照）在真实数据上**从未运行**；`applied` 也无法审计。
...
| 级别 | 缺陷 | 证据 |
|---|---|---|
| **P0** | **课内温故环三处断点**（计划被初始化覆盖 / 从未进入 LLM / 结果摘取饿死） | §3.2【A】 |
