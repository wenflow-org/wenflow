...
## 0. Agent / Skill 全景与缘由（见 AGENT_SKILL_MANUAL.md）

> 本指南讲**怎么做**（选型 → scaffold → 接线 → 加字段 → 门禁 → 发布 → 测试）。
> "**为什么这样拆 Agent / Skill**"（功能缘由、设计意图、边界、五阶段数据旅程）见
> [`AGENT_SKILL_MANUAL.md`](./AGENT_SKILL_MANUAL.md)（开发者向；**开发文档，不是平台功能/能力说明**）。
> 新增/改造前建议先读它建立意图，再回本文走流程；若改动改变了模块职责或边界，请同步更新该文对应条目。
