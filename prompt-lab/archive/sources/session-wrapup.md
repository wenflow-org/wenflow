...
|-------|------|----------|-------------|
| （标签化纯文本） | text | yes | 非 JSON 输入，每个字段一个【标签】分区 |

分区结构：
- 基本：【学科】【主题】【时长】【学生消息数】【助教消息数】
- 任务与路径：【任务类型】【任务标题】【任务说明】【路径标题】【路径摘要】【路径背景】(JSON)
- 结构与结束：【课堂最终状态】(JSON)【课堂事件历史】(JSON)【阶段轨迹】(JSON)【结束原因】
- 知识与状态：【知识点状态】(JSON)【知识点变化】(JSON delta)【学习状态】(JSON 或"无")
- 证据与对话：【课堂证据】(JSON)【最近对话片段】（编号对话摘要）
- 结尾指令：主模式"同时输出 summary 与 evaluation"；评估回退模式只输出 evaluation 对象
