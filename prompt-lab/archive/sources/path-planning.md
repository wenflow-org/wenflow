...
|-------|------|----------|-------------|
| （标签化纯文本） | text | yes | 非 JSON 输入，按分区组织：原始学习目标/学习主题/目标水平等标签行 + 可选分区 |

分区结构：
- 标签行：原始学习目标、学习主题、目标水平，可选 具体应用场景、学习重点、可用时间、总学习周期
- 用户确认的方案轮廓（可选）：学习方向/首个产出/关键阶段/暂不纳入范围/学习方式
- 路径前置清洗结果（可选，JSON）：normalizedInput 主真相源（problemSpace/successCriteria/confirmedProposal/planningHints）
- 完整对话历史（可选）
- 路径重调模式（可选）：重调模式/触发来源/冻结任务 + 学习者重调投影 JSON + 重调要求
- 强制要求/重要要求/生成前自检：生成约束清单

normalizedInput 子字段（经由"路径前置清洗结果"JSON 提供）：
