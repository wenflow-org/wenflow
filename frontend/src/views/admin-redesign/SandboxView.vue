          ...
          <div class="sbx__col">
            ...
            <ul v-if="agent.inputChannels.length" class="sbx__list mono">
              <li v-for="c in agent.inputChannels" :key="c.path" class="sbx__li">
                ...
                <span class="sbx__src" :class="`sbx__src--${c.source}`">{{ c.source }}</span>
              </li>
            </ul>
...
.sbx__handoff { flex-shrink: 0; padding: 0 8px; border-radius: 999px; background: #eef5ff; color: var(--mk-blue, #3478f6); font-size: 10.5px; font-weight: 700; }
