<template>
  <div class="teaching-sessions-page">
    <div class="page-header">
      <div>
        <h2 class="page-title">教学会话调试</h2>
        <p class="page-subtitle">查看课后 wrapup 与 advisory，验证 learner 和建议链路是否符合预期。</p>
      </div>
      <div class="toolbar">
    ...
    <el-table v-loading="loading" :data="sessions" stripe>
      ...
      <el-table-column label="Wrapup" min-width="260">
        <template #default="{ row }">
          <div v-if="row.wrapup" class="summary-cell">
            ...
            <span>总结：{{ row.wrapup.sources?.summary || '-' }}</span>
            <span>评估：{{ row.wrapup.sources?.evaluation || '-' }}</span>
...
.topic-cell, .user-cell, .summary-cell { display: grid; gap: 4px; }
.topic-cell span, .user-cell span, .summary-cell span { color: #6b7280; font-size: 12px; }
