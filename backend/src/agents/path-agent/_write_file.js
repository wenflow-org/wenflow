...
export class PathAdjustmentEngine {
  ...
  async adjustMilestones(
    path: any,
    adjustment: PathAdjustment,
    context: AgentContext
  ): Promise<{ milestones: MilestoneOutput[]; adjustments: PathAdjustment[] }> {
    ...
    switch (adjustment.type) {
      case 'insert':
        return await this.insertMilestone(milestones, adjustment, context);
  ...
  async adjustSubtasks(
    ...
    switch (adjustment.type) {
      case 'insert':
        return await this.insertSubtask(milestones, milestoneIndex, subtasks, adjustment, context);
  ...
  private async insertMilestone(
    ...
    const newMilestone = adjustment.content as MilestoneOutput || 
      await this.generateMilestoneForInsertion(milestones, adjustment, context);
  ...
  private async insertSubtask(
    ...
    const newSubtask = adjustment.content as SubtaskOutput ||
      await this.generateSubtaskForInsertion(milestone, subtasks, adjustment, context);
  ...
  private async generateMilestoneForInsertion(
    milestones: MilestoneOutput[],
    adjustment: PathAdjustment,
    context: AgentContext
  ): Promise<MilestoneOutput | null> {
    const client = getOpenAIClient();
    
    const existingTopics = milestones.map((m, i) => \`\${i + 1}. \${m.title}\`).join('\n');
    
    const systemPrompt = \`你是一位课程设计专家。需要在现有学习路径中插入一个新的里程碑。

现有里程碑主题：
\${existingTopics}

插入位置：第 \${adjustment.position} 个里程碑之前
插入原因：\${ADJUSTMENT_PROMPTS[adjustment.reason]}

请生成一个合适的新里程碑，返回JSON格式：
{
  "title": "里程碑标题",
  "description": "里程碑描述",
  "goal": "里程碑目标",
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          subtasks: milestoneData.subtasks.map((s: any, i: number) => ({
      ...
      console.error('[PathAdjustment] Failed to generate milestone:', error);
    }
    
    return null;
  }
  
  private async generateSubtaskForInsertion(
    milestone: MilestoneOutput,
    existingSubtasks: SubtaskOutput[],
    adjustment: PathAdjustment,
    context: AgentContext
  ): Promise<SubtaskOutput | null> {
    const client = getOpenAIClient();
    
    const existingSubtasksList = existingSubtasks.map((t, i) => \`\${i + 1}. \${t.title} (\${t.type})\`).join('\n');
    
    const systemPrompt = \`你是一位课程设计专家。需要在现有里程碑中插入一个新的子任务。

里程碑主题：\${milestone.title}
里程碑描述：\${milestone.description || '无'}
现有子任务：
\${existingSubtasksList}

插入原因：\${ADJUSTMENT_PROMPTS[adjustment.reason]}

请生成一个合适的新子任务，返回JSON格式：
{
  "title": "子任务标题",
  "type": "reading|practice|project|quiz",
  "estimatedMinutes": 30,
