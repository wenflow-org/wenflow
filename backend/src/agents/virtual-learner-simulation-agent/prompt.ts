...
11. 你是同一个稳定身份的人，但不同会话里可以带着不同的具体目标来。不要把自己表现成一生只围绕一个永恒目标的人。

12. 不要机械复述对方刚刚的总结、提炼或转述，除非你是在纠正其中某一处不准确。

13. 不要总是用“我理解你…”“是的，你说得对…”“你提到的是…”这类模板化、客服式、访谈式开头。

14. 更常见的自然做法是：直接回答，被问到细节就给细节，被问到例子就给例子，被误解时就纠正一点。
...
function buildSessionGoalSection(goalContext?: SessionGoalContext): string {
  if (!goalContext) {
    return '当前没有显式的本次目标切片，请仅根据对话与身份做最合理模拟。';
  }

  const lines: string[] = [];
  if (goalContext.sourceGoal) lines.push(`初始目标表述：${goalContext.sourceGoal}`)
  if (goalContext.surfaceGoal) lines.push(`表层目标：${goalContext.surfaceGoal}`)
  if (goalContext.realProblem) lines.push(`这次真正想解决的问题：${goalContext.realProblem}`)
  if (goalContext.motivation) lines.push(`这次学习动机：${goalContext.motivation}`)
  if (goalContext.urgencyHint) lines.push(`紧迫性线索：${goalContext.urgencyHint}`)
  if (goalContext.constraints?.length) lines.push(`这次限制条件：${goalContext.constraints.join('、')}`)
  if (goalContext.expectedOutcome) lines.push(`期望结果：${goalContext.expectedOutcome}`)
  if (goalContext.acceptanceHint) lines.push(`达到什么程度算有用：${goalContext.acceptanceHint}`)
  if (goalContext.personaGoalBias?.length) lines.push(`这个人一贯偏好的目标倾向：${goalContext.personaGoalBias.join('、')}`)

  return lines.length ? lines.join('\n') : '当前没有显式的本次目标切片，请仅根据对话与身份做最合理模拟。';
}
...
function buildStageBehaviorSection(context: SimulationContext): string {
  if (context.currentStage === 'goal') {
    return [
      ...
      '稳定的是你的身份，不稳定的是你这次来要解决的具体事情。当前对话只围绕这一次的目标切片展开。',
  ...
  if (context.currentStage === 'path') {
    return [
      ...
      '判断重点优先看：起点是否适合、是否解决这次真实问题、时间是否扛得住、第一步是否愿意开始。'
    ].join('\n');
  }
  ...
  return [
    ...
    '同时你会隐含地判断：当前内容是否真的服务于你这次来要解决的问题。'
  ].join('\n');
}
...
function buildConcernPoolSection(context: SimulationContext): string {
  ...
  const personaConcernPool = context.goalState?.personaConcernPool;
  const sessionGoalConcernPool = context.goalState?.sessionGoalConcernPool;
  ...
  const personaPrimary = (personaConcernPool?.primary || []).filter((item: string) => !disclosedConcerns.includes(item));
  const goalPrimary = (sessionGoalConcernPool?.primary || []).filter((item: string) => !disclosedConcerns.includes(item));

  return [
    `已暴露关切：${disclosedConcerns.length ? disclosedConcerns.join('；') : '暂无'}`,
    `稳定身份顾虑：${personaPrimary.length ? personaPrimary.join('；') : '暂无'}`,
    `本次目标顾虑：${goalPrimary.length ? goalPrimary.join('；') : '暂无'}`,
...
export function buildSimulationPrompt(context: SimulationContext): string {
  ...
  const goalSection = [
    `默认目标种子：${profile.learningGoal}`,
    '这代表这个人通常会带着怎样的学习诉求进入系统，但不等于每次会话都只会说这一件事。'
  ].join('\n');
  const sessionGoalSection = buildSessionGoalSection(context.sessionGoalContext);
  ...
  let prompt = DEFAULT_SIMULATION_PROMPT
    ...
    .replace('{SESSION_GOAL_SECTION}', sessionGoalSection)
