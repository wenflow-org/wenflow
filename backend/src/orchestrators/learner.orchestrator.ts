...
const ORCHESTRATOR_ID = 'learner-orchestrator'
...
class LearnerOrchestrator {
  ...
  private async refreshLearnerSnapshots(event: LearningEvent): Promise<void> {
    ...
    const pathId = typeof event.data?.pathId === 'string' ? event.data.pathId : undefined
...
export const learnerOrchestrator = new LearnerOrchestrator()
export default learnerOrchestrator
