...
async function loadRoutings(): Promise<CachedRoutings> {
  ...
  const fieldMap = new Map<string, { promptRole: string; valueType: string }>();
  for (const f of fields) {
    fieldMap.set(f.fieldId, { promptRole: f.promptRole, valueType: f.valueType });
  }

  const byAgent = new Map<string, Map<string, FieldRoutingRow>>();
  for (const r of routings) {
    let inner = byAgent.get(r.agentId);
    if (!inner) {
      inner = new Map();
      byAgent.set(r.agentId, inner);
    }
...
export async function getAgentRoutings(
  ...
  const inner = routings.byAgent.get(agentId);
...
export async function getFieldRender(
  ...
  const routings = await loadRoutings();
  const r = routings.byAgent.get(agentId)?.get(fieldId);
...
export async function dispatchGoalEnvelope(
  ...
  const agentId = input.agentId || 'goal-conversation';
  const routings = await loadRoutings();
...
export async function pickGoalHandoffFields(
  result: any,
  downstream: 'requirement' | 'path'
): Promise<GoalToPathHandoffOutput> {
  const routings = await loadRoutings();
  const inner = routings.byAgent.get('goal-conversation') || new Map();
