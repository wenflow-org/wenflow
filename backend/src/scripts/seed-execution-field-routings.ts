 ...
 *   - teaching-agent：聚合编排，handoff 到 profile 阶段（累积画像）
 *
...
export const TEACHING_FIELD_ROUTING_FIELDS: SeedField[] = [
  ...
  { fieldId: 'control.checkpoint', promptRole: 'control-signal', valueType: 'object', description: '检查点（question/type/options/hint），间隔满足时可选输出' },
  { fieldId: 'control.completionCandidateEvidence', promptRole: 'hidden-inference', valueType: 'object', description: '完成候选的内部证据（供编排层门禁，不面向前端）' },

  // === skill:teaching-turn 输入通道（teaching-agent 编排注入） ===
  // pathInRawOutput 以回合装配源 { session, teachingState, context } 为根
  { fieldId: 'learner.learnerProjection', promptRole: 'soft-info', valueType: 'object', pathInRawOutput: 'context.learnerProjection', description: '学习者教学投影（toTeachingProjection）' },
  { fieldId: 'knowledge.state', promptRole: 'soft-info', valueType: 'object', pathInRawOutput: 'session.knowledgeState', description: '知识看板当前状态（points/currentPoint）' },
  { fieldId: 'controls.teachingControlContext', promptRole: 'soft-info', valueType: 'object', pathInRawOutput: 'teachingState.teachingControlContext', description: '教学控制上下文（priority/allow* 标志）' },
  { fieldId: 'classroomContext', promptRole: 'soft-info', valueType: 'object', pathInRawOutput: 'teachingState.classroomContext', description: '课堂上下文（上轮持久化）' },
  { fieldId: 'visibleDialogueContext', promptRole: 'soft-info', valueType: 'array<object>', pathInRawOutput: 'session.messages', description: '最近可见对话（role/content）' },
  ...
  { fieldId: 'wrapup.evaluation.sessionLss', promptRole: 'hidden-inference', valueType: 'number', description: '本次会话 LSS（学习压力评分，8-10 高压力）' },
  { fieldId: 'wrapup.evaluation.sessionKtl', promptRole: 'hidden-inference', valueType: 'number', description: '本次会话 KTL（知识获得质量，8-10 好）' },
  { fieldId: 'wrapup.evaluation.sessionLf', promptRole: 'hidden-inference', valueType: 'number', description: '本次会话 LF（疲劳负担，8-10 高疲劳）' },
  { fieldId: 'wrapup.evaluation.confidence', promptRole: 'hidden-inference', valueType: 'number', description: '本次评估的置信度' },
  { fieldId: 'wrapup.evaluation.reasoning', promptRole: 'hidden-inference', valueType: 'string', description: '评估依据（引用 1-2 个关键证据）' },
  { fieldId: 'wrapup.summary.keyTakeaways', promptRole: 'public-reply', valueType: 'array<string>', description: '收获列表' },
  { fieldId: 'wrapup.summary.actionPlan', promptRole: 'public-reply', valueType: 'array<string>', description: '行动列表（含检索式自测）' },
  { fieldId: 'wrapup.summary.evaluationHighlights', promptRole: 'public-reply', valueType: 'object', description: '亮点与改进（strengths/improvements）' },
  { fieldId: 'wrapup.summary.metricInterpretation', promptRole: 'derived-presentation', valueType: 'object', description: '本节/长期指标解读' },
  { fieldId: 'wrapup.summary.summaryVersion', promptRole: 'derived-presentation', valueType: 'string', description: 'summary 结构版本（固定 v2）' },
  ...
  { fieldId: 'guidance.pathHint', promptRole: 'public-reply', valueType: 'string', description: '路径相关提示' },
  { fieldId: 'guidance.paceHint', promptRole: 'public-reply', valueType: 'string', description: '节奏相关提示' },
  { fieldId: 'guidance.emptyStateCopy', promptRole: 'public-reply', valueType: 'string', description: '空态文案' },
  { fieldId: 'guidance.warningCopy', promptRole: 'public-reply', valueType: 'string', description: '预警文案' },
];

export const TEACHING_FIELD_ROUTINGS: SeedRouting[] = [
  // teaching-turn 输出
  { agentId: 'skill:teaching-turn', fieldId: 'reply', render: 'visible', handoff: ['teaching-agent'], internal: false, accumulate: false },
  ...['analysis.levelScore', 'analysis.confusionPoints',
  ...
  // cognitiveLevel/understanding 同链交付 peer（peer-reinforcement 消费）
  { agentId: 'skill:teaching-turn', fieldId: 'analysis.cognitiveLevel', render: 'hidden', handoff: ['teaching-agent', 'skill:peer-reinforcement'], internal: false, accumulate: true, visibilityPreset: 'agent-internal' },
  { agentId: 'skill:teaching-turn', fieldId: 'analysis.understanding', render: 'hidden', handoff: ['teaching-agent', 'skill:peer-reinforcement'], internal: false, accumulate: true, visibilityPreset: 'agent-internal' },
  ...
  { agentId: 'skill:teaching-turn', fieldId: 'control.checkpoint', render: 'visible', handoff: ['teaching-agent'], internal: false, accumulate: false, notes: '可选输出：满足间隔时才产生' },
  { agentId: 'skill:teaching-turn', fieldId: 'control.completionCandidateEvidence', render: 'hidden', handoff: ['teaching-agent'], internal: true, accumulate: false, visibilityPreset: 'agent-internal', notes: '编排层门禁内部证据，不面向前端' },

  // teaching-agent → skill:teaching-turn 输入通道（编排注入，非 LLM 产出）
  ...['learner.learnerProjection', 'knowledge.state', 'controls.teachingControlContext',
      'classroomContext', 'visibleDialogueContext'].map(fieldId => ({
    agentId: 'teaching-agent' as const, fieldId,
    render: 'hidden' as RenderValue, handoff: ['skill:teaching-turn'],
  ...
  ...['wrapup.summary.topicSummary', 'wrapup.summary.knowledgeSummary',
      ...
      'wrapup.summary.practiceAdvice', 'wrapup.summary.keyTakeaways',
      'wrapup.summary.actionPlan', 'wrapup.summary.evaluationHighlights'].map(fieldId => ({
    agentId: 'skill:session-wrapup' as const, fieldId,
    render: 'visible' as RenderValue, handoff: ['teaching-agent'],
    internal: false, accumulate: true,
  })),
  { agentId: 'skill:session-wrapup', fieldId: 'wrapup.summary.metricInterpretation', render: 'hidden', handoff: ['teaching-agent'], internal: false, accumulate: true, visibilityPreset: 'agent-internal' },
  { agentId: 'skill:session-wrapup', fieldId: 'wrapup.summary.summaryVersion', render: 'hidden', handoff: ['teaching-agent'], internal: false, accumulate: false, visibilityPreset: 'agent-internal' },
  ...['wrapup.evaluation.sessionLss', 'wrapup.evaluation.sessionKtl',
      'wrapup.evaluation.sessionLf', 'wrapup.evaluation.confidence',
      'wrapup.evaluation.reasoning'].map(fieldId => ({
    agentId: 'skill:session-wrapup' as const, fieldId,
    render: 'hidden' as RenderValue, handoff: ['teaching-agent'],
    internal: false, accumulate: true,
    visibilityPreset: 'agent-internal' as const
  ...
  ...['guidance.headline', 'guidance.subtitle', 'guidance.todayActions', 'guidance.nextStep',
      'guidance.pathHint', 'guidance.paceHint', 'guidance.emptyStateCopy', 'guidance.warningCopy'].map(fieldId => ({
...
export async function ensureTeachingFieldRoutings(systemPrisma: PrismaClient): Promise<FieldRoutingBootstrapResult> {
  ...
  for (const c of TEACHING_FIELD_ROUTING_CONTRACTS) {
  ...
  for (const f of TEACHING_FIELD_ROUTING_FIELDS) {
  ...
  for (const r of TEACHING_FIELD_ROUTINGS) {
