...
export const GOAL_FIELD_ROUTING_FIELDS: SeedField[] = [
  ...
  // hard-required
  { fieldId: 'understanding.surface_goal', promptRole: 'hard-required', valueType: 'string', description: '用户最初表述的"想学什么"原话', systemLocked: true, snakeName: 'surface_goal', camelName: 'surfaceGoal' },
  { fieldId: 'understanding.real_problem', promptRole: 'hard-required', valueType: 'string', description: '深层真实问题（回溯后收敛）', systemLocked: true },
  { fieldId: 'understanding.available_resources.time_budget', promptRole: 'hard-required', valueType: 'string', description: '时间预算（几周/几月）', systemLocked: true },
  { fieldId: 'understanding.available_resources.time_horizon', promptRole: 'soft-info', valueType: 'string', description: '时间视角（紧迫 vs 从容）' },
  { fieldId: 'understanding.success_criteria.observable_result', promptRole: 'hard-required', valueType: 'string', description: '学习成功的可观察结果', systemLocked: true },

  // soft-info
  { fieldId: 'understanding.current_baseline.level', promptRole: 'soft-info', valueType: 'string', description: '当前水平评估' },
  { fieldId: 'understanding.current_baseline.evidence', promptRole: 'soft-info', valueType: 'string', description: '当前水平的证据' },
  { fieldId: 'understanding.success_criteria.acceptance_check', promptRole: 'soft-info', valueType: 'string', description: '验收检查方法' },
  { fieldId: 'understanding.available_resources.time_per_session', promptRole: 'soft-info', valueType: 'string', description: '每次学习时长' },
  { fieldId: 'understanding.constraints_and_boundaries', promptRole: 'soft-info', valueType: 'string', description: '约束与边界', bindings: { accumulate: true } },
  { fieldId: 'understanding.pain_points', promptRole: 'soft-info', valueType: 'string', description: '当前痛点' },
  { fieldId: 'understanding.motivation', promptRole: 'soft-info', valueType: 'string', description: '学习动机' },
  { fieldId: 'understanding.urgency', promptRole: 'soft-info', valueType: 'string', description: '紧急程度' },
  { fieldId: 'understanding.scenario', promptRole: 'soft-info', valueType: 'string', description: '学习场景' },
  { fieldId: 'understanding.deadline_text', promptRole: 'soft-info', valueType: 'string', description: '截止日期文本' },

  // hidden-inference
  { fieldId: 'understanding.background_experience', promptRole: 'hidden-inference', valueType: 'string', description: '背景经验推断（prompt 明确不展示给前端）' },
  { fieldId: 'understanding.learning_signal', promptRole: 'hidden-inference', valueType: 'string', description: '学习信号（隐式推断）' },

  // proposal-output
  { fieldId: 'confirmedProposal.learning_direction', promptRole: 'proposal-output', valueType: 'string', description: '确认的学习方向' },
  { fieldId: 'confirmedProposal.first_deliverable', promptRole: 'proposal-output', valueType: 'string', description: '第一个交付物' },
  { fieldId: 'confirmedProposal.key_stages', promptRole: 'proposal-output', valueType: 'string', description: '关键阶段' },
  { fieldId: 'confirmedProposal.out_of_scope', promptRole: 'proposal-output', valueType: 'string', description: '不在此次学习的范围' },

  // public-reply（对话 UI 相关）
  { fieldId: 'userVisible', promptRole: 'public-reply', valueType: 'string', description: '给用户看的内容（适合 LLM 聊天 UI）' },
  { fieldId: 'goalConversation.nextQuestions', promptRole: 'public-reply', valueType: 'array<string>', description: '下一轮建议问题' },
  { fieldId: 'goalConversation.quickReplies', promptRole: 'public-reply', valueType: 'array<string>', description: '快捷回复选项' },

  // control-signal
  { fieldId: 'core.conversationId', promptRole: 'control-signal', valueType: 'string', description: '对话 ID' },
  { fieldId: 'core.stage', promptRole: 'control-signal', valueType: 'string', description: '对话阶段' },
  { fieldId: 'core.confidence', promptRole: 'control-signal', valueType: 'number', description: '收敛置信度', notes: 'internal — 仅作 UI 进度条' },
  { fieldId: 'core.isCompleted', promptRole: 'control-signal', valueType: 'boolean', description: '对话是否完成' },
];
...
export const GOAL_FIELD_ROUTINGS: SeedRouting[] = [
  ...
  { agentId: 'goal-agent', fieldId: 'understanding.real_problem', render: 'visible', handoff: ['path'], internal: false, accumulate: false, notes: 'path description 的最终兜底' },
  { agentId: 'goal-agent', fieldId: 'understanding.background_experience', render: 'hidden', handoff: ['path'], internal: false, accumulate: false, visibilityPreset: 'agent-internal' },
  { agentId: 'goal-agent', fieldId: 'understanding.learning_signal', render: 'hidden', handoff: ['path'], internal: false, accumulate: false, visibilityPreset: 'agent-internal' },
  { agentId: 'goal-agent', fieldId: 'confirmedProposal.key_stages', render: 'visible', handoff: ['path'], internal: false, accumulate: false },
];
