...
if users:
    ...
    c.execute("SELECT id, name, description, subject, totalWeeks, estimatedHours, aiPromptTemplate FROM learning_paths WHERE userId = ? ORDER BY createdAt DESC", (user_id,))
    paths = c.fetchall()
    ...
    for p in paths:
        ...
        print(f'  - weeks 详情:')
        for w in weeks_data:
            t = w["title"][:40] if w["title"] else "无标题"
            print(f'      周{w["weekNumber"]}: {t}')
        ...
        if p["aiPromptTemplate"]:
            try:
                ...
                wp = ai.get("weeklyPlan", [])
                print(f'  - AI返回 weeklyPlan 长度: {len(wp)}')
                if wp:
                    print('  - AI weeklyPlan 内容:')
                    for i, w in enumerate(wp[:12]):
                        theme = w.get("theme", "无主题")
                        print(f'      周{i+1}: {theme[:50]}')
                    if len(wp) > 12:
                        print(f'      ... 还有 {len(wp) - 12} 周')
